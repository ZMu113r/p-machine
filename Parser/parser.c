...
int factor()
{
    ...
    {
        printf("Error 18: Token Not Recognized.\n");
        exit(18);
    }
}
...
void statement()
{
    ...
    if(strcmp(token, "identsym") == 0)
    {
        ...
        int answer = lookUp(2, token, 0, 0, 0);
...
void constant_Declaration()
{
    ...
    while(condition)
    {
        ...
        //lets convert the token to a number using atoi function
        int temp_val = atoi(token);

        //insert into hash table
        insertHash(1, temp_val, temp_name, 0, addr++);
...
void proc_Declaration()
{
    ...
    //this gets the proc name
    getNextToken();

    //inserts into the hashtable
    insertHash(3, token, 0, 0, addr++);
